...
import com.coupon.go.api_interface.SetCouponInterface;
...
public class SetCouponRequestHandler extends RequestHandler implements IServerResponse {
    ...
    public SetCouponRequestHandler(boolean isJSONObj, boolean shouldShowDialog, Activity activity, CouponApplication context) {
        super(activity);
    ...
    public void callService(final Map<String,String> values, final IServerResponse iLoginSuccess) {
        try {
            if(Util.isMobileInternetOn(activity)){
                ...
                SetCouponInterface clueInterface = adapter.create(SetCouponInterface.class);

                Util.showLog("URL : ", UrlConstant.BASE_URL + UrlConstant.SET_COUPON + " ?????");
                ...
                if(values != null && values.size() > 0){
                    try{
                        ...
                        ir_code = values.get(ParamConstant.PARAM_IR_CODE);
                ...
                clueInterface.setCoupon(device_id, ir_code, new Callback<Response>() {
                    @Override
